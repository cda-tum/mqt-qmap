...
void TargetMetricHandler::makeTargetMetric(const SynthesisData& data,
  ...
  if (useMaxSat) {
    ...
    case TargetMetric::FIDELITY:
      makeFidelityMetric(data, configuration.architecture,
                         configuration.fidelityScaling);
      break;
    }
  }
}
...
void TargetMetricHandler::makeFidelityMetric(
    const SynthesisData& data, const Architecture& architecture,
    const std::uint32_t fidelityScaling) {
  if (!architecture.isArchitectureAvailable() ||
      !architecture.isCalibrationDataAvailable()) {
  ...
  for (const auto& edge : data.reducedCM) {
    logicbase::LogicTerm fidelity = logicbase::LogicTerm(
        (std::log(fidelityTable[edge.first][edge.second])) * fidelityScaling);
    const auto a =
        std::find(data.qubitChoice.begin(), data.qubitChoice.end(), edge.first);
    const auto b = std::find(data.qubitChoice.begin(), data.qubitChoice.end(),
                             edge.second);
  ...
  for (std::size_t a = 0; a < data.nqubits; ++a) {
    logicbase::LogicTerm fidelity = logicbase::LogicTerm(
        (std::log(singleQubitFidelities[a])) * fidelityScaling);
...
void TargetMetricHandler::updateResults(const Configuration& configuration,
  ...
  case TargetMetric::TWO_QUBIT_GATES:
    if ((results.sat &&
         (results.twoQubitGates) < (currentResults.twoQubitGates)) ||
        (currentResults.twoQubitGates) == 0) {
      currentResults = results;
    }
    break;
  case TargetMetric::DEPTH:
    if ((results.sat && results.depth < currentResults.depth) ||
        currentResults.depth == 0) {
      currentResults = results;
    }
    break;
  case TargetMetric::FIDELITY:
    if ((results.sat && results.fidelity > currentResults.fidelity) ||
        currentResults.fidelity == 0) {
      currentResults = results;
    }
    break;
  }
