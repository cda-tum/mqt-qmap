...
#include <cstddef>
namespace cs {
void cs::HeuristicStrategy::runHeuristicStrategy(
    const CouplingMap& reducedCM, const QubitSubset& qubitChoice,
...
void HeuristicStrategy::runSplitIter(const CouplingMap&   reducedCM,
  ...
  while (!stopping) {
    ...
    for (size_t i = 0; i * circuitSplit < circuit.getNindividualOps();
      ...
      if (totalResult.result == logicbase::Result::UNSAT) {
        ...
        circuitSplit += std::max(
            1.0, circuitSplit * configuration.circuitSplittingIncrease);
    ...
    for (const auto& r : results) {
      ...
      for (const auto& gate : splitResult) {
        localResultCircuit.insert(localResultCircuit.end(), gate->clone());
      }
    }
  ...
  synthesizer.optimalResults.resultTableaus.emplace_back(
      *configuration.targetTableau);
  synthesizer.optimalResults.singleQubitGates =
      getSingleQubitOperations(circuit);
  synthesizer.optimalResults.twoQubitGates =
      circuit.getNindividualOps() - synthesizer.optimalResults.singleQubitGates;
  synthesizer.optimalResults.result = logicbase::Result::SAT;
}
void HeuristicStrategy::runSplinter(
    int i, std::size_t circSplit, std::size_t split,
    const CouplingMap& reducedCM, const QubitSubset& qubitChoice,
    qc::QuantumComputation& circuit, std::shared_ptr<Results> r,
    CliffordSynthesizer* opt, const Configuration& configuration) {
  const Tableau targetTableau{circuit, 0,
                              static_cast<std::size_t>((i + 1U)) * circSplit};
  const Tableau initTableau{circuit, 0,
                            static_cast<std::size_t>(i) * circSplit};
