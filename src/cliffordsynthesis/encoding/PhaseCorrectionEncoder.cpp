...
namespace cs::encoding{
  ...
  void PhaseCorrectionEncoder::createVariables() {
    ...
    for (std::size_t i = 0U; i < S; ++i) {
      ...
      for(const auto& pauli: PAULIS) {
        const std::string gName = "g_" + std::to_string(i) + "_" + toString(pauli);
  ...
  void PhaseCorrectionEncoder::encodePauliConstraints() {
    ...
    for (std::size_t q = 0U; q < N; ++q) {
      ...
      for(std::size_t row = 0U; row < S; ++row) {
        ...
        iChange.emplace_back(logicbase::LogicTerm::ite(paulis[q][0], LogicTerm(false), LogicTerm(false)));
        DEBUG() << "Encoding X constraint.";
        xChange.emplace_back(logicbase::LogicTerm::ite(paulis[q][1], zCol[row], LogicTerm(false)));
        DEBUG() << "Encoding Y constraint.";
        yChange.emplace_back(logicbase::LogicTerm::ite(paulis[q][2], !(zCol[row]==xCol[row]), LogicTerm(false)));
        DEBUG() << "Encoding Z constraint.";
        zChange.emplace_back(logicbase::LogicTerm::ite(paulis[q][3], xCol[row], LogicTerm(false)));
      }
      splitXorR(iChange);
      splitXorR(xChange);
      splitXorR(yChange);
      ...
      lb->assertFormula(paulis[q][0] || paulis[q][1] || paulis[q][2] || paulis[q][3]);
    }
    DEBUG() << "Assert Phase.";
    for(std::size_t row = 0U; row < S; ++row) {
  ...
  if (xorHelpers.size() == 1) {
    ...
    for(std::size_t row = 0U; row < S; ++row) {
      lb->assertFormula(xorHelpers.back()[row] == (changes[row] != xorHelpers[xorHelpers.size() - 2][row]));
    }
  }
}

  LogicVector PhaseCorrectionEncoder::vectorFromBitset(const std::bitset<64>& bs) const{
    LogicVector result{};
    result.reserve(S);
    for (std::size_t i = 0U; i < S; ++i) {
