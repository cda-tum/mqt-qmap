...
void cs::GateEncoding::makeSingleGateEncoding(const SynthesisData& data) {
  ...
  for (std::size_t gateStep = 1; gateStep < data.timesteps + 1; ++gateStep) {
    ...
    for (std::size_t a = 0; a < data.nqubits; ++a) {
      ...
      changes = changes && (data.r[gateStep] ==
                            (data.r[gateStep - 1] ^ (data.x[gateStep - 1][a] &
                                                     data.z[gateStep - 1][a])));
      ...
      changes =
          changes && (data.r[gateStep] ==
                      (data.r[gateStep - 1] ^
                       (data.x[gateStep - 1][a] &
                        (data.x[gateStep - 1][a] ^ data.z[gateStep - 1][a]))));
      changes = logicbase::LogicTerm::implies(
          data.gS[gateStep][Gates::toIndex(Gates::GATES::Sdag)][a], changes);
      data.lb->assertFormula(changes);
      ...
      changes = changes && (data.r[gateStep] ==
                            (data.r[gateStep - 1] ^ data.z[gateStep - 1][a]));
      changes = logicbase::LogicTerm::implies(
          data.gS[gateStep][Gates::toIndex(Gates::GATES::X)][a], changes);
      data.lb->assertFormula(changes);
      ...
      changes = changes && (data.r[gateStep] ==
                            (data.r[gateStep - 1] ^ (data.z[gateStep - 1][a]) ^
                             data.x[gateStep - 1][a]));
      changes = logicbase::LogicTerm::implies(
          data.gS[gateStep][Gates::toIndex(Gates::GATES::Y)][a], changes);
      data.lb->assertFormula(changes);
      ...
      changes = changes && (data.r[gateStep] ==
                            (data.r[gateStep - 1] ^ data.x[gateStep - 1][a]));
      changes = logicbase::LogicTerm::implies(
          data.gS[gateStep][Gates::toIndex(Gates::GATES::Z)][a], changes);
      data.lb->assertFormula(changes);
                     ...
                     (data.r[gateStep - 1] ^
                      ((data.x[gateStep - 1][a] & data.z[gateStep - 1][b]) &
                       ((data.x[gateStep - 1][b] ^ data.z[gateStep - 1][a]) ^
                        logicbase::LogicTerm((1 << data.nqubits) - 1,
                                             data.nqubits)))));
          makeCNOTConstraints(data, a, b, gateStep, changes);
          for (std::size_t c = 0; c < data.nqubits;
               ++c) { // All other entries do not change
            if (a == c || b == c) {
              continue;
            }
            changes =
                changes && (data.x[gateStep][c] == data.x[gateStep - 1][c]);
            changes =
                changes && (data.z[gateStep][c] == data.z[gateStep - 1][c]);
          }

          changes = logicbase::LogicTerm::implies(
              data.gTwoQubit[gateStep][a][b], changes);
          data.lb->assertFormula(changes);
        }
        ++bIt;
      }
  ...
  for (std::size_t gateStep = 1; gateStep < data.timesteps + 1; ++gateStep) {
    ...
    for (std::size_t a = 0; a < data.nqubits; ++a) {
      ...
      rChanges = logicbase::LogicTerm::ite(
          ...
          rChanges ^ (data.x[gateStep - 1][a] & data.z[gateStep - 1][a]),
          rChanges);
      changes = logicbase::LogicTerm::implies(data.gS[gateStep][1][a], changes);
      ...
      rChanges = logicbase::LogicTerm::ite(
          ...
          rChanges ^ (data.z[gateStep - 1][a] ^ data.x[gateStep - 1][a]),
          rChanges);
      changes = logicbase::LogicTerm::implies(
          data.gS[gateStep][Gates::toIndex(Gates::GATES::Y)][a], changes);
      data.lb->assertFormula(changes);
...
void cs::GateEncoding::makeGateEncoding(
    const SynthesisData& data, const cs::Configuration& configuration) {
...
void cs::GateEncoding::makeNotChangingSet(const cs::SynthesisData& data,
                                          std::size_t a, std::size_t gateStep,
...
void cs::GateEncoding::makeSdagConstraints(const cs::SynthesisData& data,
                                           std::size_t a, std::size_t gateStep,
                                           encodings::LogicTerm& changes) {
  changes = (data.z[gateStep][a] ==
             (data.z[gateStep - 1][a] ^ data.x[gateStep - 1][a]));
  changes = changes && (data.x[gateStep][a] == data.x[gateStep - 1][a]);
}
void cs::GateEncoding::makeXConstraints(const cs::SynthesisData& data,
                                        std::size_t a, std::size_t gateStep,
                                        encodings::LogicTerm& changes) {
  changes = (data.z[gateStep][a] == data.z[gateStep - 1][a]);
  changes = changes && (data.x[gateStep][a] == data.x[gateStep - 1][a]);
}
void cs::GateEncoding::makeYConstraints(const cs::SynthesisData& data,
                                        std::size_t a, std::size_t gateStep,
                                        encodings::LogicTerm& changes) {
  changes = (data.z[gateStep][a] == data.z[gateStep - 1][a]);
  changes = changes && (data.x[gateStep][a] == data.x[gateStep - 1][a]);
}
void cs::GateEncoding::makeZConstraints(const cs::SynthesisData& data,
                                        std::size_t a, std::size_t gateStep,
                                        encodings::LogicTerm& changes) {
  changes = (data.z[gateStep][a] == data.z[gateStep - 1][a]);
  changes = changes && (data.x[gateStep][a] == data.x[gateStep - 1][a]);
}
void cs::GateEncoding::makeSingleGateConsistency(
    const cs::SynthesisData& data) {
  ...
  for (std::size_t gateStep = 1; gateStep < data.timesteps + 1; ++gateStep) {
    ...
    for (std::size_t a = 0; a < data.nqubits; ++a) {
      ...
      for (std::size_t b = 0; b < data.nqubits; ++b) {
        ...
        changes = changes + data.gTwoQubit[gateStep][a][b] +
                  data.gTwoQubit[gateStep][a][b];
      }
    }
    changes =
        changes < logicbase::LogicTerm(static_cast<int>(data.nqubits + 1));
    data.lb->assertFormula(changes);
  }
}
void cs::GateEncoding::makeCNOTConstraints(const cs::SynthesisData& data,
