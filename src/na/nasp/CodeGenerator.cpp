...
auto CodeGenerator::coordFromDiscrete(
  ...
  return {static_cast<std::int64_t>(x) * dx + h * minAtomDist,
          2 * zoneDist + static_cast<std::int64_t>(y - 2) * dy + v * minAtomDist};
}
...
auto CodeGenerator::generate(
    ...
    const std::size_t minEntanglingY, const std::size_t maxEntanglingY)
  -> NAComputation {
  ...
  // initialize atoms in SLM and load required ones into AOD
  {
    ...
    for (const auto& q : result.front().getQubits()) {
      auto pos = std::make_shared<Point>(coordFromDiscrete(
          q.getX(), q.getY(), q.getH(), q.getV(), maxHOffset, maxVOffset,
          minEntanglingY, maxEntanglingY));
    ...
    if (!loadPositions.empty()) {
      code.emplaceBack<
        NAShuttlingOperation>(LOAD, loadPositions, loadPositions);
    }
  }
  ...
  if (affectedQubits.size() != input.getNqubits() || ops.size() !=
      affectedQubits.size()) {
  ...
  code.emplaceBack(
      std::make_unique<
        NAGlobalOperation>(FullOpType{RY, 0}, std::vector{PI_2}));
  ...
  if (result.front().isRydberg()) {
    code.emplaceBack(
        std::make_unique<NAGlobalOperation>(FullOpType{Z, 1}));
    ...
    for (const auto& g : result.front().getGates()) {
      const auto& it =
          std::find_if(
              ...
              executableSet.end(),
              [g](const auto& v) {
                if (v->getOperation()->getType() == Z && v->
                    getOperation()->getNcontrols() == 1) {
  ...
  for (std::size_t t = 1; t < result.numStages(); ++t) {
    ...
    if (!loadEndPositions.empty()) {
      code.emplaceBack<
        NAShuttlingOperation>(LOAD, loadStartPositions, loadEndPositions);
    }
    if (!startPositions.empty()) {
      code.emplaceBack<
        NAShuttlingOperation>(MOVE, startPositions, endPositions);
    }
  ...
  if (!executableSet.empty()) {
    code.emplaceBack(
        std::make_unique<NAGlobalOperation>(FullOpType{RY, 0},
                                            std::vector{-PI_4}));
    while (!executableSet.empty()) {
      ...
      code.emplaceBack(
          std::make_unique<NALocalOperation>(
              FullOpType{RZ, 0}, std::vector{PI}, pos));
      v->execute();
    }
    code.emplaceBack(
        std::make_unique<NAGlobalOperation>(
            FullOpType{RY, 0}, std::vector{PI_4}));
  }
  return code;
}
} // namespace na