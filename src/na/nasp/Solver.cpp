...
auto NASolver::getExactNumTransfersConstraints() const -> std::vector<expr> {
  ...
  if (numTransfers.has_value() && numTransfers.value() != 0) {
    ...
    constraints.emplace_back(
        ult(transfers[numTransfers.value() - 1], ctx.bv_val(
                numStages, minBitsToRepresentUInt(numStages))));
  }
  return constraints;
}
...
auto NASolver::getAffectedByRydbergBeamConstraint(const std::uint16_t q,
  ...
  case Storage::Bottom:
    return ule(stages[t].getQubit(q).getY(), ctx.bv_val(
                   maxEntanglingY, minBitsToRepresentUInt(maxY)));
  case Storage::TwoSided:
    return ule(minEntanglingY,
               stages[t].getQubit(q).getY()) &&
           ule(stages[t].getQubit(q).getY(), ctx.bv_val(
                   maxEntanglingY, minBitsToRepresentUInt(maxY)));
  }
}
...
auto NASolver::getValidRydbergTransitionConstraints(
  ...
  for (std::uint16_t i = 0; i < numQubits; ++i) {
    ...
    constraints.emplace_back(
        implies(
            ...
            stages[t].getQubit(i).getA() == stages[t + 1].getQubit(i).getA()));
  ...
  for (std::uint16_t i = 0; i <= maxR; ++i) {
    constraints.emplace_back(
        implies(
            getRydbergStageConstraint(t),
            !stages[t].getLoadRow(i)));
    constraints.emplace_back(
        implies(
            getRydbergStageConstraint(t),
            !stages[t].getStoreRow(i)));
  }
  return constraints;
}
    ...
    for (std::uint16_t j = 0; j < numQubits; ++j) {
      ...
      constraints.emplace_back(
          implies(
              ...
              (ult(stages[t].getQubit(i).getX(),
                   stages[t].getQubit(j).getX()) ||
               ((stages[t].getQubit(i).getX() == stages[t].getQubit(j).getX())
                && slt(stages[t].getQubit(i).getH(), stages[t].getQubit(j).
                       getH()))) ==
              ult(stages[t + 1].getQubit(i).getC(), stages[t + 1].getQubit(j).
                  getC())));
      constraints.emplace_back(
          implies(
              getTransferStageConstraint(t) &&
              stages[t + 1].getQubit(i).getA() &&
              stages[t + 1].getQubit(j).getA(),
              (ult(stages[t].getQubit(i).getY(),
                   stages[t].getQubit(j).getY()) ||
               ((stages[t].getQubit(i).getY() == stages[t].getQubit(j).getY())
                &&
                slt(stages[t].getQubit(i).getV(),
                    stages[t].getQubit(j).getV())))
              ==
              ult(stages[t + 1].getQubit(i).getR(), stages[t + 1].getQubit(j).
                  getR())));
    }
  }
  return constraints;
...
auto NASolver::getCircuitExecutionConstraints(
  ...
  if (mindOpsOrder) {
    ...
    constraints.reserve(
        3 * numGates + numStages * numQubits * (numQubits - 1) / 2);
  } else {
    constraints.reserve(
      ...
      for (std::uint16_t j = i + 1; j < numQubits; ++j) {
        if (const auto& it = pairToGates.find({i, j});
          ...
          for (const auto& gate : gatesForPair) {
            ...
            auto hDiffSign =
                ashr(hDiff, static_cast<signed int>(minBitsToRepresentInt(
                                maxHOffset)) - 1);
            auto absHDiff = (hDiff ^ hDiffSign) - hDiffSign;
            auto vDiff =
                stages[t].getQubit(j).getV() - stages[t].getQubit(i).getV();
            auto vDiffSign =
                ashr(vDiff, static_cast<signed int>(minBitsToRepresentInt(
                                maxVOffset)) - 1);
          ...
          expr premisses = getRydbergStageConstraint(t) &&
                           ...
                           getAffectedByRydbergBeamConstraint(j, t);
          for (const auto& gate : gatesForPair) {
            premisses = premisses && gate != ctx.bv_val(
                            t, minBitsToRepresentUInt(numStages));
          }
          constraints.emplace_back(implies(premisses,
                                           getHaveDifferentPositionConstraint(
                                               i, j, t)));
        } else {
          constraints.emplace_back(
              implies(
...
auto NASolver::getTransferStageConstraint(const std::uint16_t t) const -> expr {
  if (numTransfers.has_value()) {
    ...
    for (const auto& transfer : transfers) {
      clauses = clauses || transfer == ctx.bv_val(t, minBitsToRepresentUInt(
                                                      numStages));
    }
    return clauses;
  }
...
auto NASolver::getValidStageConstraints(
  ...
  for (std::uint16_t i = 0; i < numQubits; ++i) {
    ...
    constraints.emplace_back(
        ...
        // getX()) &&
        ule(stages[t].getQubit(i).getX(), ctx.bv_val(
                maxX, minBitsToRepresentUInt(maxX))));
    // 0 <= y <= maxY
    constraints.emplace_back(
        // 0 <= stages[t].getQubit(i).
        // getY() &&
        ule(stages[t].getQubit(i).getY(), ctx.bv_val(
                maxY, minBitsToRepresentUInt(maxY))));
    // For AOD: 0 <= c <= maxC
    constraints.emplace_back(
        implies(
            stages[t].getQubit(i).getA(),
            // 0 <= stages[t].getQubit(i).
            // getC() &&
            ule(stages[t].getQubit(i).getC(), ctx.bv_val(
                    maxC, minBitsToRepresentUInt(maxC)))));
    // For AOD: 0 <= r <= maxR
    constraints.emplace_back(
        implies(
            stages[t].getQubit(i).getA(),
            // 0 <= stages[t].getQubit(i).
            // getR() &&
            ule(stages[t].getQubit(i).getR(), ctx.bv_val(
                    maxR, minBitsToRepresentUInt(maxR)))));
    // For AOD: - maxHOffset <= h <= maxHOffset
    constraints.emplace_back(
        implies(
            stages[t].getQubit(i).getA(),
            sle(ctx.bv_val(-static_cast<signed int>(maxHOffset),
                           minBitsToRepresentInt(maxHOffset)), stages[t].
                getQubit(i)
                .getH()) &&
            sle(stages[t].getQubit(i).getH(),
                maxHOffset)));
    ...
    constraints.emplace_back(
        implies(
            ...
            sle(ctx.bv_val(-static_cast<signed int>(maxVOffset),
                           minBitsToRepresentInt(maxVOffset)), stages[t].
                getQubit(i)
                .getV()) &&
            sle(stages[t].getQubit(i).getV(),
                maxVOffset)));
    ...
    constraints.emplace_back(
        implies(
            !stages[t].getQubit(i).getA(),
            stages[t].getQubit(i).getC() == ctx.bv_val(
                0, minBitsToRepresentUInt(maxC)) &&
            stages[t].getQubit(i).getR() == ctx.bv_val(
                0, minBitsToRepresentUInt(maxR)) &&
            stages[t].getQubit(i).getH() == ctx.bv_val(
    ...
    for (std::uint16_t j = 0; j < numQubits; ++j) {
      ...
      constraints.emplace_back(
          implies(
              stages[t].getQubit(i).getA() &&
              stages[t].getQubit(j).getA(),
              (ult(stages[t].getQubit(i).getX(),
                   stages[t].getQubit(j).getX()) ||
               ((stages[t].getQubit(i).getX() == stages[t].getQubit(j).getX())
                &&
                slt(stages[t].getQubit(i).getH(),
                    stages[t].getQubit(j).getH())))
              ==
...
auto NASolver::init(const std::uint16_t newMaxX, const std::uint16_t newMaxY,
  ...
  if (minEntanglingY == 0 && maxEntanglingY < maxY) {
    ...
    throw std::invalid_argument(
        "One sided storage zone is only supported below the entangling zone (higher Y).");
  }
}
...
auto NASolver::solve(
  ...
  for (const auto& c : getCircuitExecutionConstraints(
  ...
  for (const auto& stage : stages) {
    const bool rydberg = nTrans == numTransfers
                         || model.eval(transfers[nTrans]).as_uint64()
                         != stage.getT();
    if (!rydberg) { ++nTrans; }
    ...
    for (std::uint16_t i = 0; i < numQubits; ++i) {
      resultQubits.emplace_back(
          ...
          model.eval(bv2int(
              stage.getQubit(i).getH(),
              true)).get_numeral_int(),
          model.eval(bv2int(
              stage.getQubit(i).getV(),
              true)).get_numeral_int()
          );
    }
