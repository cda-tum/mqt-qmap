  ...
  for (const auto& op : logicQC) {
    if (op->isGlobalOperation()) {
      ...
      for (std::size_t i = 0; i < shuttlingSize; ++i) {
        const auto s = sop.getStart().at(i);
        const auto e = sop.getEnd().at(i);
        for (std::int64_t r = 0; r < rows; ++r) {
          for (std::int64_t c = 0; c < cols; ++c) {
    ...
    if (op->isShuttlingOperation()) {
      ...
      if (shuttlingOp.getType() == MOVE) {
        ...
        for (std::size_t i = 0; i < shuttlingOp.getStart().size(); ++i) {
          const auto  start = *shuttlingOp.getStart().at(i);
          const auto  end   = *shuttlingOp.getEnd().at(i);
        ...
        for (std::size_t i = 0; i < shuttlingOp.getStart().size(); ++i) {
          auto       start = *shuttlingOp.getStart().at(i);
          auto       end   = *shuttlingOp.getEnd().at(i);
          const auto dx    = end.x - start.x;
    ...
    return std::all_of(
        ...
        [&](const auto& qubit) {
          ...
          case Atom::PositionStatus::UNDEFINED:
            ...
            // selected zones
            return std::any_of(
                placement.at(qubit).zones.cbegin(), placement.at(qubit).zones.cend(),
                [&](const auto& z) {
          ...
          case Atom::PositionStatus::DEFINED:
            ...
            return arch.isAllowedLocallyAt({op->getType(), 0},
                                           *placement.at(qubit).currentPosition);
          default:
            qc::unreachable();
          }
        });
...
auto NAMapper::getMisplacement(const std::vector<Atom>&      initial,
  ...
  for (std::size_t i = 0; i < target.size(); ++i) {
    if (initial.at(target.at(i)).positionStatus ==
        Atom::PositionStatus::UNDEFINED) {
      continue;
    }

    if (i < indexOfQ && initial.at(target.at(i)).currentPosition->x >
  ...
  { // load atoms that are not already shuttling
    ...
    for (const auto q : qubits) {
      if (currentlyShuttling.find(q) == currentlyShuttling.cend()) {
        ...
        currentFreeSites.at(*arch.getSiteAt(*placement.at(q).currentPosition)) = true;
    ...
    for (const auto q : qubits) {
      if (currentlyShuttling.find(q) != currentlyShuttling.cend()) {
        ...
        if (n == currentlyShuttling.size() - notStoredLeft) {
          ...
          n -= 1;
        } else if (j < sitesInRow.size() and currentFreeSites.at(sitesInRow.at(j))) {
        ...
        if (currentlyShuttling.find(p) != currentlyShuttling.cend()) {
          ...
          if (placement.at(p).positionStatus == Atom::PositionStatus::DEFINED) {
            if (placement.at(p).currentPosition->y == y and
              ...
              currentFreeSites.at(*arch.getSiteAt(*placement.at(p).currentPosition)) =
                  true;
            ...
            while (!free) {
              ...
              if (initialFreeSites.at(site) and
                  std::find(
                      placement.at(p).zones.cbegin(), placement.at(p).zones.cend(),
                      arch.getZoneOfSite(site)) != placement.at(p).zones.cend()) {
            ...
            if (free) {
              ...
              *placement.at(p).initialPosition = {freeX, y};
              initialFreeSites.at(*arch.getSiteAt(*placement.at(p).initialPosition)) =
                  false;
              // pick up p
              pickUpOrder.erase(
                  std::remove(pickUpOrder.begin(), pickUpOrder.end(), p),
  ...
  while (it != executableSet.end()) {
    ...
    if (config.getMethod() == NAMappingMethod::Naive) {
      ...
      const Point& target = arch.getPositionOfSite(arch.getSitesInZone(
          *arch.getPropertiesOfOperation({op->getType(), 1}).zones.begin()).at(0));
      end.x += d;
