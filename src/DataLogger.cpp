...
void DataLogger::clearLog() {
  for (const auto& entry : std::filesystem::directory_iterator(dataLoggingPath)) {
...
void DataLogger::logArchitecture(Architecture& arch) {
  ...
  if (!of.good()) {
    deactivated = true;
    std::cerr << "[data-logging] Error opening file: " << dataLoggingPath << "architecture.json" << std::endl;
    return;
  }
  nlohmann::json json;
  ...
  if (arch.isFidelityAvailable()) {
    ...
    fidelity["single_qubit_fidelity_costs"] = arch.getSingleQubitFidelityCosts();
...
void DataLogger::openNewLayer(std::size_t layer) {
  ...
  for (std::size_t i=searchNodesLogFiles.size(); i<=layer; ++i) {
    searchNodesLogFiles.push_back(std::ofstream(dataLoggingPath+"nodes_layer_"+std::to_string(i)+".csv"));
    if (!searchNodesLogFiles.at(i).good()) {
      deactivated = true;
      std::cerr << "[data-logging] Error opening file: " << dataLoggingPath << "layer_" << i << ".json" << std::endl;
      return;
    }
  }
};

void DataLogger::logFinalizeLayer(std::size_t layer, const qc::CompoundOperation& ops, 
  ...
  if (!searchNodesLogFiles.at(layer).is_open()) {
    std::cerr << "[data-logging] Error: layer " << layer << " has already been finalized" << std::endl;
    return;
  }
  searchNodesLogFiles.at(layer).close();
  
  auto of = std::ofstream(dataLoggingPath+"layer_"+std::to_string(layer)+".json");
  if (!of.good()) {
    deactivated = true;
    std::cerr << "[data-logging] Error opening file: " << dataLoggingPath << "layer_" << layer << ".json" << std::endl;
    return;
  }
  nlohmann::json json;
  ...
  for (const auto& sw : swaps) {
    for (const auto& s : sw) {
      ...
      if (s.op != qc::OpType::SWAP) {
        of << " " << s.op;
        if (s.middleAncilla != std::numeric_limits<decltype(s.middleAncilla)>::max()) {
          of << " " << s.middleAncilla;
        }
      }
      of << ",";
...
void DataLogger::logMappingResult(MappingResults& result) {
  ...
  if (!of.good()) {
    deactivated = true;
    std::cerr << "[data-logging] Error opening file: " << dataLoggingPath << "mapping_result.json" << std::endl;
    return;
  }
  nlohmann::json json;
  ...
  config["add_measurements_to_mapped_circuit"] = result.config.addMeasurementsToMappedCircuit;
  ...
  stats["additional_gates"] =
      static_cast<std::make_signed_t<decltype(result.output.gates)>>(result.output.gates) -
      static_cast<std::make_signed_t<decltype(result.input.gates)>>(result.input.gates);
  ...
  if (result.config.method == Method::Exact) {
    ...
    for (std::size_t i=0; i<result.layerHeuristicBenchmark.size(); ++i) {
      ...
      jsonLayerBenchmark["average_branching_factor"] = layerBenchmark.averageBranchingFactor;
      jsonLayerBenchmark["effective_branching_factor"] = layerBenchmark.effectiveBranchingFactor;
    }
  }
  of << json.dump(2);
  of.close();
...
void DataLogger::close() {
  for (std::size_t i=0; i<searchNodesLogFiles.size(); ++i) {
    if (searchNodesLogFiles.at(i).is_open()) {
      std::cerr << "[data-logging] Error: layer " << i << " was not finalized" << std::endl;
