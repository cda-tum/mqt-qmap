...
protected:
  ...
  auto placeQubitIntermediate() -> void {
    const auto t_p = std::chrono::system_clock::now();
    VertexMatchingPlacer intermediatePlacer(static_cast<T*>(this)->getArchitecture(),
        static_cast<T*>(this)->getQubitMapping().front());
    intermediatePlacer.run(static_cast<T*>(this)->getQubitMapping(),
...
private:
  auto placeTrivial() -> void {
    std::vector<std::unique_ptr<SLM>>::const_iterator slmIt =
        static_cast<T*>(this)->getArchitecture().storageZone.cbegin();
    const SLM* slm = slmIt->get();
    std::size_t c = 0;
    // decide whether to begin with row 0 or row n
    const double dis1 =
        static_cast<T*>(this)->getArchitecture().nearestEntanglementSiteDistance(
            slm, 0, c);
    const double dis2 =
        static_cast<T*>(this)->getArchitecture().nearestEntanglementSiteDistance(
            slm, slm->nRows - 1, c);
    std::int64_t r = dis1 < dis2 ? 0 : static_cast<std::int64_t>(slm->nRows) - 1;
  ...
  public:
    ...
    auto run(const std::vector<
             ...
             const std::vector<std::unordered_set<std::size_t>>& reuseQubits) -> void {
      this->listReuseQubits = reuseQubits;
      std::cout << "[INFO] ZAC: Minimum-weight-full-matching-based "
                   "intermediate placement: Start\n";
      placeGate(qubitMapping, listGate, 0, false);
      for (std::size_t layer = 0; layer < listGate.size(); ++layer) {
        if (dynamicPlacement) {
          placeQubit(listGate, layer, false);
        } else {
          mapping.emplace_back(qubitMapping[0]);
        }
        if (layer + 1 < listGate.size()) {
          placeGate(mapping, listGate, layer + 1, false);
        }
        if (!reuseQubits[layer].empty()) {
          ...
          if (layer + 1 < listGate.size()) {
            placeGate({mapping[mapping.size() - 4], mapping[mapping.size() - 1]}, listGate, layer + 1, true);
            filterMapping(layer);
          }
        }
      }
    ...
    auto filterMapping(const std::size_t layer) -> void {
      ...
      for (std::size_t q = 0; q < lastGateMapping.size(); ++q) {
        if (lastGateMapping[q] != gateMapping[q]) {
          const auto slmIdx1 =
              std::get<0>(lastGateMapping[q])->entanglementId->front().get();
          const auto slmIdx2 =
              std::get<0>(qubitMapping[q])->entanglementId->front().get();
          const std::tuple key{slmIdx1, std::get<1>(lastGateMapping[q]),
                               slmIdx2, std::get<1>(qubitMapping[q])};
          const double dis = architecture.distance(
              std::get<0>(lastGateMapping[q]),
              std::get<1>(lastGateMapping[q]),
        ...
        if (qubitMapping[q] != gateMapping[q]) {
          const auto slmIdx1 =
              std::get<0>(gateMapping[q])->entanglementId->front().get();
          const auto slmIdx2 =
              std::get<0>(qubitMapping[q])->entanglementId->front().get();
          const std::tuple key{slmIdx2, std::get<1>(qubitMapping[q]),
                               slmIdx1, std::get<1>(gateMapping[q])};
          const double dis = architecture.distance(
    ...
    /// @param listQubitMapping the initial mapping before the Rydberg stage
    /// @param listTwoGateLayer gates to be executed in the current Rydberg
    /// stage
    auto placeGate(
      ...
      const auto& qubitMapping = listQubitMapping[layer > 0 ? 1 : 0];
      const std::vector<std::tuple<const SLM*, size_t, size_t>>& gateMapping =
          layer > 0 ? listQubitMapping[0]
      ...
      std::vector<std::size_t> listData;
      ...
      for (std::size_t i = 0; i < listGate.size(); ++i) {
        ...
        if (testReuse && (listReuseQubits[layer - 1].find(q1) !=
          ...
          const auto slmIdx =
              std::get<0>(location)->entanglementId->front().get();
          ...
          std::vector<std::tuple<const SLM*, std::size_t, std::size_t>>
              listNearestsite{};
          listNearestsite.emplace_back(architecture.nearestEntanglementSite(
          ...
          for (const auto& nearestSite : listNearestsite) {
            ...
            if (high_c - low_c < 2 * expandFactor) {
              const auto heightGap = static_cast<std::size_t>(
                  std::ceil(listGate.size() / (high_c - low_c)) -
                  expandFactor);
            ...
            if (high_r - low_r < 2 * expandFactor) {
              const auto widthGap = static_cast<std::size_t>(
                  std::ceil(listGate.size() / (high_r - low_r)) -
                  expandFactor);
        ...
        for (const auto& site : setNearbysite) {
          ...
          if (dictReuseQubitNeighbor.find(q1) !=
          ...
          if (q3) {
            dis3 = architecture.distance(std::get<0>(qubitMapping[*q3]),
                                         std::get<1>(qubitMapping[*q3]),
                                         std::get<2>(qubitMapping[*q3]),
                                         std::get<0>(site), std::get<1>(site),
                                         std::get<2>(site));
          }
          listRowCoo.emplace_back(idxRydberg);
          listColCoo.emplace_back(i);
          if (std::get<1>(qubitMapping[q1]) ==
                  std::get<1>(qubitMapping[q2]) &&
              std::get<0>(qubitMapping[q1]) ==
                  std::get<0>(qubitMapping[q2])) {
      ...
      std::vector matrix(
          listRydberg.size(),
          std::vector<std::optional<double>>(listGate.size(), std::nullopt));
      ...
      for (std::size_t idxRydberg = 0; idxRydberg < matching.size();
           ++idxRydberg) {
        const auto q0 = listGate[matching[idxRydberg]]->first;
        const auto q1 = listGate[matching[idxRydberg]]->second;
        const auto& site = listRydberg[idxRydberg];
        if (testReuse && (listReuseQubits[layer - 1].find(q0) !=
                           listReuseQubits[layer - 1].end())) {
          tmpMapping[q0] = gateMapping[q0];
          if (site == gateMapping[q0]) {
            tmpMapping[q1] = {std::get<0>(site)->entanglementId->back().get(),
                               std::get<1>(site), std::get<2>(site)};
          } else {
            tmpMapping[q1] = site;
          }
          ...
          if (site == gateMapping[q1]) {
            tmpMapping[q0] = {std::get<0>(site)->entanglementId->back().get(),
                               std::get<1>(site), std::get<2>(site)};
          } else {
            tmpMapping[q0] = site;
          }
    ...
    auto placeQubit(
      ...
      for (const std::unique_ptr<SLM>& slmId :
           architecture.storageZone) {
      ...
      for (std::size_t q = 0; q < lastGateMapping.size(); ++q) {
        ...
        if (isEmptyStorageSite.find(arrayId) !=
            isEmptyStorageSite.end()) {
      ...
      std::forward<decltype(args)>(args)...);
        ...
        for (const auto& site : setNearbysite) {
          ...
          for (const auto& neighborQ : dictQubitInteraction[q]) {
            ...
            if (!std::get<0>(siteNeighborQ)->entanglementId) {
  ...
  return architecture.exactSlmLocation(
      std::forward<decltype(args)>(args)...);
        ...
        std::vector costMatrix (qubitToPlace.size(),
                                std::vector<std::optional<double>>(
                                    listStorage.size(), std::nullopt));
