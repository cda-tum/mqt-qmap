...
protected:
   ...
   */
  void graphMatching(std::vector<CoordIndex>& qubitIndices, std::vector<CoordIndex>& hwIndices, const qc::DAG& dag);
  ...
  std::pair<MoveCombs, std::vector<std::pair<MoveComb, const qc::Operation*>> > getAllMoveCombinationsWithOp();
  /**
...
public:
  ...
  void reset() {
    hardwareQubits =
        HardwareQubits(arch, parameters.initialMapping, std::vector<CoordIndex>(), std::vector<CoordIndex>(), parameters.seed);
  }
