...
TEST(Functionality, DataLogger) {
  ...
  if (!archFile.is_open()) {
    ...
    EXPECT_EQ(fidelityJson["single_qubit_fidelity_costs"],
              architecture.getSingleQubitFidelityCosts());
    EXPECT_EQ(fidelityJson["swap_fidelity_costs"],
              architecture.getSwapFidelityCosts());
    EXPECT_EQ(fidelityJson["two_qubit_fidelities"],
              architecture.getFidelityTable());
    EXPECT_EQ(fidelityJson["two_qubit_fidelity_costs"],
...
// TODO: deactivated because of long runtime, reactivate once premature
// search-termination for fidelity-aware mapping is implemented
